...
bool compile(const Path& output, const Source &source)
{
    (void)output;
    (void)source;
    StopWatch sw;
    ...
    std::shared_ptr<clang::TargetOptions> targetOptions(new clang::TargetOptions(compilerInstance.getTargetOpts()));
...
#if CLANG_VERSION_MINOR < 3 || CLANG_VERSION_MINOR > 5
                                                                   targetOptions
#else
                                                                   &targetOptions
#endif
