...
void Project::index(const std::shared_ptr<IndexerJob> &job)
{
    ...
    if (job->flags & IndexerJob::Compile) {
        ...
        while (it != mSources.end()) {
            ...
            if (found) {
                it->second.flags &= ~Source::Enabled;
                ++it;
                continue;
            }
            ...
            if (it->first == key) {
                it->second.flags |= Source::Enabled;
                found = true;
                if (it->second.compareArguments(source)) {

                }
            } else if (it->second.flags & Source::Enabled) {
                it->second.flags &= ~Source::Enabled;
            } else if (disallowMultiple || it->second.compareArguments(source)) {
                if (!found) {
                    if (!(it->second.flags & Source::Enabled)) {
                        it->second.flags |= Source::Enabled;
                    }
                    found = true;
                } else if (it->second.flags & Source::Enabled) {
...
int Project::startDirtyJobs(Dirty *dirty, const UnsavedFiles &unsavedFiles)
{
    List<Source> toIndex;
    for (const auto &source : mSources) {
        if (source.second.flags & Source::Enabled && dirty->isDirty(source.second)) {
